...
const isPromotionActive = (promo: Promotion) => {
  const now = new Date()
  const startDate = promo.promoStartDate.toJSDate()
  const endDate = promo.promoEndDate.toJSDate()
...
const togglePromotionStatus = async (promo: Promotion) => {
  ...
  if (confirm(`Êtes-vous sûr de vouloir ${action} cette promotion ?`)) {
    try {
      // Calculer les nouvelles dates en fonction de l'action
      const now = new Date()
      let newStartDate = promo.promoStartDate
      let newEndDate = promo.promoEndDate
      
      if (isActive) {
        // Désactiver : mettre la date de fin à hier
        newEndDate = DateTime.fromJSDate(new Date(now.getTime() - 24 * 60 * 60 * 1000))
      } else {
        // Activer : mettre la date de début à aujourd'hui et la date de fin à 30 jours
        newStartDate = DateTime.fromJSDate(now)
        newEndDate = DateTime.fromJSDate(new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000))
      }
      
      const response = await fetch('/dashboard/promotions/edit', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          promoStartDate: newStartDate.toFormat('yyyy-MM-dd'),
          promoEndDate: newEndDate.toFormat('yyyy-MM-dd')
        })
      })
