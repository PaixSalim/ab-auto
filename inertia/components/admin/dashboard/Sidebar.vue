...
const navigation = computed(() => {
  ...
  if (isAdmin.value) {
    ...
    const adminItems = [
      ...(can('view_users') || can('create_users') ? [{ label: 'Utilisateurs', route: '/dashboard/users', icon: 'i-mdi-account-cog-outline' }] : []),
      ...(can('view_roles') || can('manage_roles') ? [{ label: 'Rôles', route: '/dashboard/roles', icon: 'i-mdi-shield-account-outline' }] : []),
      ...(isSuperAdmin.value ? [{ label: 'Permissions', route: '/dashboard/permissions', icon: 'i-mdi-key-chain-variant' }] : []),
    ]
    ...
    <div class="p-8 pb-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{{ isAdmin ? 'Administration' : 'Vendeur' }}</p>
        </div>
      </div>
    </div>
