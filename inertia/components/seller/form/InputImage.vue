...
const images = defineModel<{ file: File; type: 'image'; preview: string }[]>('images')
...
<template>
  <div class="mb-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4" @dragover.prevent @drop.prevent="handleDrop">
      ...
      <!-- Upload Button -->
      <label
        class="aspect-square rounded-lg border-2 border-dashed border-gray-300 hover:border-primary transition-colors flex flex-col items-center justify-center cursor-pointer bg-gray-50 hover:bg-gray-100"
      >
        <div class="i-mdi-cloud-upload text-primary text-3xl mb-2"></div>
        <span class="text-gray-600 text-center text-sm">Déposer ou cliquez pour ajouter</span>
        <input
          type="file"
          multiple
          accept=".jpg, .jpeg, .png"
